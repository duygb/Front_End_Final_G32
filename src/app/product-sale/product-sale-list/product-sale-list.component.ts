...
export class ProductSaleListComponent implements OnInit {
  @Output() onMyClick = new EventEmitter();
  allSaleProducts!: SaleProduct[];
  @Input() saleProductsSorted!: SaleProduct[];
  saleProducts!: SaleProduct[];
  @Output() onProductsSorted = new EventEmitter();
  ...
  getSaleProductsSorted(value: HTMLOptionElement) {
    let getValue = value.value;
    value.disabled = true;
    this.pagination.indexPagination = 1;
    this.myServerHttpService.getProductsSortBy(getValue[0],getValue[1],this.pagination.indexPagination, this.pagination.visiblePage).subscribe((data) => {
      this.saleProducts = data;
      this.setPrice(this.saleProducts);
      this.router.navigate(['sale-product']);
    })
    /* this.onProductsSorted.emit(value);
    console.log(this.saleProducts);
    console.log(this.saleProductsSorted);
    this.saleProducts = this.saleProductsSorted;
    this.router.navigate(['sale-product']); */
  }
